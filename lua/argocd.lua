...
function M.list_apps()
  ...
  local function fetch_and_draw()
    ...
    local res = api.api_request("get", "/api/v1/applications")
...
function M.update_app(app_name)
  ...
  -- Fetch full app data
  local res = api.api_request("get", "/api/v1/applications/" .. app_name)
  if res.status ~= 200 then
  ...
  vim.api.nvim_buf_set_keymap(edit_buf, "n", "<CR>", "", {
    ...
    callback = function()
      ...
      local patch_res = api.api_request("patch", "/api/v1/applications/" .. app_name, patch_body)
...
function M.sync_app(app_name)
  ...
  if app_status ~= "Synced" then 
    vim.ui.select({"No", "Yes"}, {
      ...
      if choice == "Yes" then
        local res = api.api_request("post", "/api/v1/applications/" .. app_name .. "/sync")
        if res.status == 200 then
...
function M.delete_app(app_name)
  ...
  vim.ui.select({"No", "Yes"}, {
    ...
    if choice == "Yes" then
      local res = api.api_request("delete", "/api/v1/applications/" .. app_name)
      if res.status == 200 then
